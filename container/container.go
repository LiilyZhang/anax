...
func (b *ContainerWorker) ResourcesCreate(agreementId string, agreementProtocol string, deployment *containermessage.DeploymentDescription, configureRaw []byte, environmentAdditions map[string]string, ms_networks map[string]string, serviceURL string, sVer string, originalAgreementId string, ms_instanceKey string) (persistence.DeploymentConfig, error) {
...
func (b *ContainerWorker) CommandHandler(command worker.Command) bool {
	...
	case *WorkloadConfigureCommand:
		...
		if ags, err := persistence.FindEstablishedAgreements(b.db, cmd.AgreementLaunchContext.AgreementProtocol, []persistence.EAFilter{persistence.UnarchivedEAFilter(), persistence.IdEAFilter(agreementId)}); err != nil {
			...
			if deploymentConfig, err := b.ResourcesCreate(agreementId, cmd.AgreementLaunchContext.AgreementProtocol, deploymentDesc, cmd.AgreementLaunchContext.ConfigureRaw, *cmd.AgreementLaunchContext.EnvironmentAdditions, ms_children_networks, serviceIdentity, sVer, agreementId, cmd.AgreementLaunchContext.MicroserviceInstanceDBKey); err != nil {
				eventlog.LogAgreementEvent(b.db, persistence.SEVERITY_ERROR,
	...
	case *ContainerConfigureCommand:
		...
		if deployment, err := b.ResourcesCreate(lc.Name, "", deploymentDesc, []byte(""), *lc.EnvironmentAdditions, ms_children_networks, serviceIdentity, sVer, lc.AgreementIds[0], lc.Name); err != nil {
