...
type PersistedServiceSecret struct {
	SvcOrgid        string   `json:"svcOrgID"`
	SvcUrl          string   `json:"svcURL"`
	SvcArch         string   `json:"svcArch"`
	SvcVersionRange string   `json:"svcVersionRange"`
	SvcSecretName   string   `json:"svcSecretName"`
	SvcSecretValue  string   `json:"svcSecretValue"`
	AgreementIds    []string `json:"agreementIds`
	TimeCreated     uint64   `json:"timeCreated"`
	TimeLastUpdated uint64   `json:"timeLasUpdated"`
}
...
func SaveSecret(db *bolt.DB, secretName string, msDefId string, msDefVers string, secretToSave *PersistedServiceSecret) error {
	glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret for def id: %v", msDefId))
	...
	glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret secretToSave.TimeCreated: %d, secretToSave.TimeLastUpdated: %d", secretToSave.TimeCreated, secretToSave.TimeLastUpdated))

	if mergedSec, ok := secretToSaveAll.SecretsMap[secretName]; ok {
		glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret mergedSec"))
		...
		glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret it is NOT mergedSec"))
		secretToSaveAll.SecretsMap[secretName] = secretToSave
	}
	secToSave := secretToSaveAll.SecretsMap[secretName]
	glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret read from map secretToSave.TimeCreated: %d, secretToSave.TimeLastUpdated: %d", secToSave.TimeCreated, secToSave.TimeLastUpdated))

	glog.V(3).Infof(fmt.Sprintf("Lily - SaveSecret func done for def id: %v, call SaveAllServiceSecrets", msDefId))
...
func SaveAllServiceSecrets(db *bolt.DB, msDefId string, secretToSaveAll *PersistedServiceSecrets) error {
	glog.V(3).Infof(fmt.Sprintf("Lily - SaveAllServiceSecretsfor def id: %v", msDefId))
	...
	glog.V(3).Infof(fmt.Sprintf("Lily - SaveAllServiceSecretsfor done def id: %v, writeErr: %v", msDefId, writeErr))
	return writeErr
}
