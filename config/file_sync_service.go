...
type FSSConfig struct {
	...
	UnixSocketFilePermissions string // the permission digit for socket file
}
...
func (c *HorizonConfig) GetFSSObjectQueueSize() uint64 {
	if c.Edge.FileSyncService.ObjectQueueBufferSize == 0 {
...
func (c *HorizonConfig) GetUnixSocketFilePermissions() string {
	if c.Edge.FileSyncService.UnixSocketFilePermissions == "" {
		return "0660"
	} else {
		return c.Edge.FileSyncService.UnixSocketFilePermissions
	}
}
